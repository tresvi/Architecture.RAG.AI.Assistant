...
//ContextReducer: https://learn.microsoft.com/en-us/semantic-kernel/concepts/ai-services/chat-completion/chat-history?pivots=programming-language-csharp
...
namespace HotelSample.RAG.AI.Assistant
{
    internal class Program
    {
        ...
        const int NRO_MSJES_CONTEXTO = 100;

        //static ChatHistoryTruncationReducer _reducer = new ChatHistoryTruncationReducer(targetCount: 15); // Keep system message and last user message
            ...
            try
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        /************* Reduccion de Mensajes ***************/
                        //var reducedMessages = await _reducer.ReduceAsync(history)!;
                        //ChatHistorySummarizationReducer _reducer2 = new ChatHistorySummarizationReducer(chatService, NRO_MSJES_CONTEXTO);
                        //var reducedMessages = await _reducer2.ReduceAsync(history)!;
